...
def save_log(log,do_print=False):
    if do_print:
        print(log)
...
def find_all_elements(driver):
    try:
        WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR,'div[class="_7jvw x2izyaf x1hq5gj4 x1d52u69"]')))
    except:
        pass
    return driver.find_elements(By.CSS_SELECTOR, value = 'div[class="_7jvw x2izyaf x1hq5gj4 x1d52u69"]')
...
def find_profile_pic(element,download):
    try:
        ...
        if download == 'yes':
            ...
            with open(f'../media/profile_pics/{name}', 'wb') as handler:
                handler.write(img_data)
        return link
    except Exception as e:
        save_log(f'on function find_profile_pic the error for date \n {e}')
...
def find_ad_videos(element,download):
    try:
        ...
        for video,poster in zip(videos_links,posters):
            ...
            if seconds < 120:
                #download videos
                import os
                if not os.path.exists(f'../media/ads_videos/{name}'):
                    if download == 'yes':
                        urllib.request.urlretrieve(video, f'../media/ads_videos/{name}')
                    ...
                    if download == 'yes':
                        ...
                        with open(f'../media/posters/{poster_name}', 'wb') as handler:
                            handler.write(img_data)
                else:
                    vids_links.append(video)
                    vids_length.append(seconds)
        ...
        save_log(f'on function get videos the error for \n {e}')
        return "" , "" , ""
...
def value_to_int(x):
    ...
    if 'M' in x:
        ...
        return 1000000
    if 'B' in x:
        return int(float(x.replace('B', '')) * 1000)
    return int(x)
...
def get_page_data(element,driver):
    ...
    try:
        ...
        for s in pages:
            text = s.find_element(By.XPATH,'.').text.replace(',','').replace('.','')
            #facebook 
            if re.search('like', text):
                FB_ID = s.find_element(By.XPATH,'./div[2]/div[1]').text
                page_likes = value_to_int(re.findall('(\S.+) like' , text)[0])
            if re.search('follower', text):
                Insta_ID = s.find_element(By.XPATH,'./div[2]/div[1]').text
                insta_followers = value_to_int(re.findall('(\S.+) follower' , text)[0])
    except Exception as e:
        error = str(e)
...
#   password="AWS$2020",
    ...
    #irelend
    host="database-2.cagcp7l7ujqg.eu-west-1.rds.amazonaws.com",
    user="admin",
    password="NEWAWS2020",
...
def start_save(driver,search_term,country= "ALL",start_date = None,end_date=None,media_type='video',limit = 'no limit',type='keyword',download ='yes'):
    if type == 'keyword':
    ...
    while len(elements)==0 and tries < 2:
        tries += 1
        save_log(f"no elements , {tries}",True)
        time.sleep(300)
        driver.refresh()
        elements = find_all_elements(driver)  #driver.find_elements(By.CSS_SELECTOR, value = 'div[class="xh8yej3"]')
    mydb = database_connect()
    mycursor = mydb.cursor(buffered=True)
    SQL_query = f"""update search_terms set last_visited =%s WHERE search_term =%s and search_type = %s and country = %s"""
    mycursor.execute(SQL_query,(datetime.now(),search_term,'keyword',country))
    mydb.commit()
    mycursor.close() 
    mydb.close()
    ...
    for element in elements[0:limit]:
        ...
        # if AD id exist just update
        mydb = database_connect()
        mycursor = mydb.cursor(buffered=True)
        SQL_query = f"""select ad_id from ads where ad_id = {AD_ID}"""
        mycursor.execute(SQL_query)
        ad = mycursor.fetchone()
        mycursor.close()
        mydb.close()
        # if ad:
        #     pass
        ...
        videos,video_length,posters = find_ad_videos(element,download)
        if len(videos)<1:
            continue
        ...
        profile_pic = find_profile_pic(element,download) #save with static id ??
        ...
        #insert if not exist             
            ...
            ,instgram_ID ,insta_followers ,static_ID ,Ads_count ,cumulative_ads_count ,days,date ,hits ,search_term ,favourite,poster,country,days_count)    
            VALUES 
            ...
            # sql_txt = f'''INSERT INTO ads (AD_ID ,Started_date ,profile_pic ,links ,videos ,videos_length,content ,Footer_text ,Footer_action ,Page_name ,AD_occurance ,Facebook_ID ,Page_likes 
            #             ,instgram_ID ,insta_followers ,static_ID ,Ads_count ,cumulative_ads_count ,days,date ,hits ,search_term ,favourite,poster,country,days_count) VALUES ({','.join(['?']*26)})'''
            # cursor.execute(sql_txt,(AD_ID ,
            #     Started_date ,
            #     profile_pic ,
            #     links ,
            #     videos ,
            ...
            # print("eroor found on insertion for ads",e)
            #update the data
            SQL_query = f"""SELECT days from ads where ad_id = {AD_ID}"""
            mycursor.execute(SQL_query)
            ad = mycursor.fetchone()
            days_count = len(set(ad[0].split(',')))
            update_tires = 0
            while update_tires < 3:
                try:
            ...
            days = concat( %s , ',' , days) ,
            hits = hits + 1 , 
            search_term = concat( search_term , ',' , %s) ,
            country =  concat( country , ',' , %s) ,
            days_count = %s
            where AD_ID = %s'''
              ,(AD_occurance ,
                time_now,
        ...
        try:
            ...
            mycursor.execute("""insert into 
                          ...
                          VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(static_ID,Facebook_ID,insta_followers,instgram_ID,Page_likes,profile_pic,Page_name,no_fav,no_block,country))
            mydb.commit()
            #sqlite save 
            # cursor.execute("""insert into 
            #               facebook_pages(static_ID,Facebook_ID,insta_followers,instgram_ID,Page_likes,profile_pic,page_name,favorites,block,country)
            #               VALUES (?,?,?,?,?,?,?,?,?,?)""",(static_ID,Facebook_ID,insta_followers,instgram_ID,Page_likes,profile_pic,Page_name,no_fav,no_block,country))
            # sqliteConnection.commit()
        except Exception as e:
            # print("eroor found on insertion for facebook_pages", e)
            update_tires = 0
            while update_tires < 3:
                
                try:
                    mycursor.execute('''UPDATE facebook_pages set 
                        Facebook_ID = %s,
                        insta_followers = %s,
                        instgram_ID = %s,
                        Page_likes = %s,
                    ...
                    time.sleep(2)
            #sqlite save 
            # cursor.execute('''UPDATE facebook_pages set 
            #             Facebook_ID = ?,
            #             insta_followers = ?,
            #             instgram_ID = ?,
            #             Page_likes = ?,
